...
public class HealthCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if (strings.length == 2) {
            ...
            } else if (PLUGIN.getOnlinePlayers().containsKey(Bukkit.getPlayer(strings[0]))) {
                Player player = Bukkit.getPlayer(strings[0]);
                if (!invites.containsKey(((Player) commandSender).getUniqueId())) {
                    if (PLUGIN.getOnlinePlayers().get(commandSender).getPost().getRoots().contains("healthCommand")) {
                        int price;
                        try {
