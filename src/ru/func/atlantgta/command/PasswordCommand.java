...
public class PasswordCommand implements CommandExecutor {
    ...
    public PasswordCommand(AtlantGTA plugin) {
        PLUGIN = plugin;
    }
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if (strings.length == 0) {
            showPassword((Player) commandSender);
            ...
            if ("accept".equals(strings[0])) {
                if (PLUGIN.getOnlinePlayers().containsKey(Bukkit.getPlayer(strings[1]))) {
    ...
    private void showPassword(Player sender) {

        AtlantPlayer atlantPlayer = PLUGIN.getOnlinePlayers().get(sender);
