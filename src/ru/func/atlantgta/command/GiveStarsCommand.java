...
public class GiveStarsCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if (commandSender instanceof Player) {
            if (!commandSender.isOp() || !PLUGIN.getOnlinePlayers().containsKey(commandSender) || !PLUGIN.getOnlinePlayers().get(commandSender).getPost().getRoots().contains("givestarsCommand")) {
        ...
        if (strings.length == 2) {
            if (PLUGIN.getOnlinePlayers().containsKey(Bukkit.getPlayer(strings[0]))) {
                try {
                    ...
                    PLUGIN.getOnlinePlayers().get(player).setStars(stars);
